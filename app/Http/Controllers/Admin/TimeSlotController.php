...
class TimeSlotController extends InfyOmBaseController
{
    ...
    public function store(CreateTimeSlotRequest $request)
    {
        ...
        if($private == "false"){
            ...
            if($time_slot_detail['health_plan_id'] == ""){
                return 'nao_foi_possivel_pelos_planos';
            }
            ...
            if(isset($timeSlot) && isset($timeSlot->id) && $timeSlot->id > 0){
                ...
                if (isset($id) && $id > 0) {
                    Flash::error('Horário salvo com sucesso!');
                    ...
                    Flash::error('Houve um conflite de agendamentos, tente novamente em alguns instantes!');
                    return redirect(URL::previous());
                }
            }else{
                $retorno = array_merge($timeSlot->toArray(), $time_slot_detail);

                Flash::error('Esse horário ja existe!');
                return redirect(URL::previous());
            }
        }else{
            unset($input['medics_select']);  $input['medics_select']   = null;
